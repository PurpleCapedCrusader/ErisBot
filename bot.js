...
bot.on("message", (message) => {
    ...
    if (message.channel.type != "dm" && message.guild.id === config.guildId) {
        for (let i = 0; i < godReactions.godReactionsArray.length; i++) {
            if (
                message.content
                    .toLowerCase()
                    .includes(godReactions.godReactionsArray[i])
            ) {
                const reactionEmoji = message.guild.emojis.cache.find(
                    (emoji) => emoji.name === godReactions.godReactionsArray[i]
                );
    ...
    if (message.content.slice(0, 4).toLowerCase() === "!iam") {
        if (message.channel.name === "eris-bot") {
            ...
            let roleRequested = message.content
                .slice(4, lastLetter)
                .toLowerCase();
    ...
    if (
        ...
        switch (args[0]) {
            ...
            case "joke":
                message.channel
                    .send(
                        `Hmmm... I'm thinking... I'll have to get back to you.`
                    )
                    .catch(console.error);
                break;
                    ...
                    if (godArray[i] == lowerCase(args[0])) {
                        ...
                        if (bot.godData[godSearched].update == "Updated") {
                            const embed = new Discord.MessageEmbed()
                                ...
                                .setColor(
                                    "0x" + bot.godData[godSearched].borderColor
                                )
                                .addField(
                                ...
                                .addField(
                                    "Ability(updated):",
                                    bot.godData[godSearched]
                                        .updatedAbilityFormatted + "\n\u200b"
                                )
                                .addField(
                                    "Ability(original):",
                                    bot.godData[godSearched]
                                        .originalAbilityFormatted + "\n\u200b"
                                )
                                .addField(
                                    "Banned Opponents:",
